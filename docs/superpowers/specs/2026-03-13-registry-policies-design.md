...
- **12 Policies** across 4 categories: Marketing, Product, Customer, Brand/Design (9 new + 1 enhanced + 2 replaced)
...
Checks that need to extract structured fields from deliverable content follow these conventions:

- **Email content** (used by `subject-line-length`, `email-craft`): First line of the file is the subject line. Second line is the preview text. Body follows after a blank line.
- **Product listings** (used by `marketplace-fields`, `required-sections`): Submitted as JSON with keys matching field names (e.g., `{"title": "...", "bullet_points": ["...", "..."], "description": "..."}`) or as markdown where H2 headings map to field names.
- **Landing pages and articles** (used by `heading-structure`, `required-sections`, `meta-length`): Submitted as markdown. First H1 is the title. Meta title and description are extracted from YAML frontmatter (`meta_title`, `meta_description`) if present, otherwise from the first H1 and first paragraph respectively.
...
| **Product** | `product-description`, `feature-announcement`, `help-article`, `content-article` |
...
- **Supported types:** `image/svg+xml`
- **Config schema:**
  - `max_text_percent: number` (default: `20`)
- **Logic:** For SVG, calculate area of `<text>` elements relative to viewBox area. Flag if text area exceeds threshold. Raster image support (via OCR or Anthropic vision API) is a future enhancement — for now, the `visual-hierarchy` subjective criterion evaluates text density on raster images.
- **Severity in policies:** warning
...
| `urgency-authenticity` | 1 | Landing pages frequently misuse urgency tactics |
...
| `urgency-authenticity` | 1 | Promotional emails are the second-highest risk for deceptive urgency |
...
| `urgency-authenticity` | 1 | Paid ads are the highest-risk channel for deceptive urgency tactics |
...
| `word-count` | `min: 50, max: 500` | warning |
| `required-sections` | `sections: ["subject", "next step"]` | warning |
...
### Policy 12: `content-article`

**Description:** SEO blog posts, thought leadership, editorial content, guides, tutorials
**Max revisions:** 3
...
| `heading-structure` | `require_h1: true, max_h1_count: 1, allow_skip_levels: false` | warning |
| `meta-length` | `title_min: 50, title_max: 60, desc_min: 140, desc_max: 160` | warning |
| `word-count` | `min: 800, max: 3000` | warning |

**Subjective criteria (pass threshold: 6.0):**

| Criterion | Weight | Rationale |
|---|---|---|
| `originality` | 3 | Content must differentiate from the sea of AI-generated commodity articles |
| `readability` | 3 | Long-form must maintain momentum across 800-3000 words |
| `seo-effectiveness` | 3 | Blog content is the primary organic acquisition channel for startups |
| `information-architecture` | 2 | Long-form demands strong structure to retain readers |
| `tone-alignment` | 2 | Blog voice builds brand personality over time |
| `customer-empathy` | 2 | Must address real questions the audience is searching for |
| `value-proposition-clarity` | 1 | Subtle — the brand's expertise is the value prop, not a product pitch |
| `brand-consistency` | 1 | Voice matters; heavy brand enforcement would feel forced in editorial |

This replaces the existing `blog-post` policy with a broader scope covering all long-form editorial content.

### Summary Matrix

| Policy | Checks | Criteria | Pass Threshold | Max Revisions |
|---|---|---|---|---|
| `landing-page` | 7 | 10 | 6.5 | 3 |
| `email-campaign` | 5 | 9 | 6.0 | 3 |
| `social-ad` | 8 | 9 | 6.5 | 3 |
| `social-post` | 6 | 8 | 5.5 | 2 |
| `product-description` | 6 | 8 | 6.5 | 3 |
| `feature-announcement` | 5 | 8 | 6.5 | 2 |
| `help-article` | 6 | 7 | 7.0 | 3 |
| `onboarding-sequence` | 6 | 8 | 6.5 | 3 |
| `support-response` | 4 | 7 | 7.0 | 2 |
| `brand-asset` | 4 | 4 | 7.0 | 2 |
| `social-graphic` | 6 | 7 | 6.5 | 2 |
| `content-article` | 6 | 8 | 6.0 | 3 |
...
**Phase 4: New policies** — create the 9 new policy manifests referencing checks and criteria
**Phase 5: Enhanced policies** — update existing `brand-asset` policy, retire old `instagram-ad` policy (superseded by `social-ad`), replace `blog-post` with `content-article`
...
- `blog-post` — replaced by `content-article` which covers a broader scope (SEO blogs, thought leadership, guides, tutorials). Retire after migration.
- `brand-asset` — enhanced in place with new criteria and updated prompt guidance.
- `instagram-ad` — superseded by `social-ad` which covers all platforms. Retire after migration.
